...
<body>
	...
	<div id="main-wrapper">	
		...
		<form class="myform" action="editp.php" method="post">
			...
			<label><font color="white" size='3'>Subject</font></label>
			<input name="sub" type="text" class="inputvalues" placeholder="Enter subject" required/>
		...
		<?php
			...
			if(mysqli_num_rows($checkrun)>0)
			{
				...
				$sub=$_POST['sub'];  $_SESSION['sub']=$sub;
				$age=$_POST['age'];  $_SESSION['age']=$age;
				$delquery = "delete from profile where id = '$id'";
				$delrun = mysqli_query($con,$delquery);
				$query="insert into profile(name,id,department,subject,age)values('$name','$id','$dept','$sub','$age')";
